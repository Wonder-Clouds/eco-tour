...
import { CreatePersonData } from '../api/personsApi'
...
export const CreatePersonModal = ({ isOpen, onClose }: Props) => {
  ...
  const [errors, setErrors] = useState<Partial<Record<keyof CreatePersonData, string>>>({})

  const handleChange = (field: keyof CreatePersonData, value: string) => {
  ...
  const validate = (): boolean => {
    const newErrors: Partial<Record<keyof CreatePersonData, string>> = {}
  ...
  const handleSubmit = () => {
    ...
    createPerson(formData, {
      onSuccess: () => {
        onClose()
        setFormData({ first_name: '', last_name: '', email: '', phone_number: '', passport_number: '', birth_date: '', nationality: '' })
        setErrors({})
      },
      onError: () => alert('Error al crear el cliente'),
    })
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto m-4">
        <div className="sticky top-0 bg-white border-b px-6 py-4 flex items-center justify-between">
          <h2 className="text-xl font-bold" style={{ color: '#085f24' }}>Nuevo Cliente</h2>
        ...
        <div className="sticky bottom-0 bg-white border-t px-6 py-4 flex justify-end gap-3">
          ...
          <button onClick={handleSubmit} disabled={isPending} className="px-4 py-2 rounded-lg text-white font-medium disabled:opacity-50" style={{ backgroundColor: '#00bf35' }}>{isPending ? 'Guardando...' : 'Crear Cliente'}</button>
        </div>
      </div>
    </div>
  )

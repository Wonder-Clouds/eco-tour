...
import { Package, CreatePackageServiceInput } from '@/types/package.type'
import {
  DndContext,
  closestCenter,
  KeyboardSensor,
...
const SortableItem = ({ id, order, serviceName, onRemove, isRemoving }: SortableItemProps) => {
  const {
    attributes,
    listeners,
    setNodeRef,
...
export const EditPackageModal = ({ isOpen, onClose, pkg }: Props) => {
  ...
  const mutations = useUpdatePackage(pkg.id)
  const { data: services, isLoading: loadingServices } = useServices()

  // Filtrar servicios disponibles basado en búsqueda (excluyendo los ya seleccionados)
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      const oldIndex = selectedServices.findIndex((item) => item.service_id === active.id)
      const newIndex = selectedServices.findIndex((item) => item.service_id === over.id)

      const reordered = arrayMove(selectedServices, oldIndex, newIndex)
        .map((item, index) => ({ ...item, order: index + 1 }))

      setSelectedServices(reordered)

      // Actualizar orden en backend
      mutations.updateOrder.mutate(reordered)
    }
  }
  ...
  const handleAddService = (serviceId: string) => {
    ...
    // Agregar al backend
    mutations.addService.mutate({ service_id: serviceId, order: newServices.length })
  }

  const handleRemoveService = (serviceId: string) => {
    if (confirm('¿Estás seguro de eliminar este servicio del paquete?')) {
      mutations.removeService.mutate(serviceId, {
        onSuccess: () => {
  ...
  const getServiceName = (serviceId: string) => {
    // Primero buscar en los servicios del paquete
    const pkgService = pkg.package_services?.find(s => s.service?.id === serviceId)
    if (pkgService?.service?.title) return pkgService.service.title

    // Luego buscar en todos los servicios
  ...
  const sortedServices = [...selectedServices].sort((a, b) => a.order - b.order)
    ...
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4" style={{ zIndex: 9999 }}>
      <div className="bg-white rounded-xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                disabled={loadingServices || mutations.addService.isPending}
              />
                  ...
                  {loadingServices ? (
                    ...
                    filteredServices.slice(0, 5).map((service) => (
                      <button
                        ...
                        disabled={mutations.addService.isPending}
          ...
          {/* Lista de Servicios con Drag and Drop */}
          {selectedServices.length > 0 && (
            <div>
              ...
              <div className="space-y-2 border border-gray-200 rounded-lg p-3">
                ...
                >
                  <SortableContext
                    items={sortedServices.map(s => s.service_id)}
                  ...
                  >
                    {sortedServices.map((service) => (
                      <SortableItem
                        ...
                        isRemoving={mutations.removeService.isPending}
                      />
                    ))}
                  </SortableContext>
                </DndContext>
        ...
        <div className="border-t border-gray-200 px-6 py-4 flex justify-end gap-3">
          ...
          <button
            ...
            disabled={mutations.updateBasic.isPending || !title.trim()}
